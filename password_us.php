...
<body id="password_us">
    <?php # Script 9.7 - password_us.php
    // Trang này cho phép người dùng thay đổi mật khẩu của họ.
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($errors)) {
            ...
            // Kiểm tra xem họ đã nhập đúng tên tài khoản kết hợp mật khẩu chưa:
            $q = "SELECT tentk FROM public.user WHERE (tentk='$tentk' AND matkhau='$p')";
            $r = @pg_query($conn, $q);
            $num = @pg_num_rows($r);
            if ($num == 1) { // Đã được thực hiện.
                ...
                $q = "UPDATE public.user SET matkhau='$np' WHERE tentk='$tentk'";
                $r = @pg_query($conn, $q);
                ...
                exit();
            } else { // Tên tài khoản / mật khẩu kết hợp không hợp lệ.
                echo '<h1>Error!</h1>
			<p class="error">
                    ...
                    <div class="card-body">
                        <form action="password.php" method="post">
                            <div class="form-group">
                                ...
                                <input class="form-control py-3" name="tentk" type="text" aria-describedby="emailHelp" placeholder="Nhập tên tài khoản..." value="<?php echo $tentk; ?>" readonly="readonly" />
                            </div>
