...
<body id="view_user">
	<?php # Script 10.5 - #5
	// Xem thông tin chi tiết của người dùng dành cho quản trị viên
	...
	echo '<h1 style="color: green">Thông tin người dùng</h1>';

	// Kiểm tra ID người dùng hợp lệ, thông qua GET hoặc POST:
	if ((isset($_GET['id'])) && (is_string($_GET['id']))) { // From view_users3.php
		$id = $_GET['id'];
	...
	$q1 = "SELECT quyentk, tentk, hoten, diachi, sdt, avatar, gtinh, tenltk
	  ...
	  WHERE tentk='$id' AND public.user.maltk = public.loaitk.maltk";
	    ...
	    <tr>
	        <td>
	            ...
	            <div style="margin-left: 10px; font-size: 13pt; margin-bottom: 5px">
	                <a href="edit_user_ad.php?id=' . $row1['tentk'] . '">Chỉnh sửa</a> |
	                <a href="delete_user.php?id=' . $row1['tentk'] . '">Xóa người dùng</a>
					<button><a class="form-control" style="background: blue; color: white;" href="view_users3.php">Quay lại</a></button>
	            </div>
	        </td> 
	    </tr>
	</table>
