...
<body id="view_user">
	<?php # Script 10.2 - xoa_user.php
	// Trang này dùng để xóa hồ sơ người dùng cá nhân.
	...
	echo '<h1 style="color: green">Xóa tài khoản</h1>';
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if ($_POST['sure'] == 'Yes') { // Xóa bản ghi.
			...
			if (pg_affected_rows($r) == 1) {
				...
				echo '</script>';
				echo '<script> window.location.replace("logout.php"); </script>'; // Đăng xuất tài khoản
			} else { // Nếu truy vấn không chạy được.
				echo '<p class="error">Không thể xóa người dùng do lỗi hệ thống.</p>';
			}
		} else { // Không có xác nhận xóa.
			echo '<p>Tài khoản KHÔNG bị xóa. <a href="view_users3.php">Quay lại</a></p>';
		}
	} else {
		...
		if (pg_num_rows($r) == 1) { // ID người dùng hợp lệ, hiển thị biểu mẫu.
			...
			echo '<div style="color: red; font-weight: bold; margin-bottom: 10px">Bạn có chắc chắn muốn xóa tài khoản này không?</div>';

			// Tạo biểu mẫu:
			echo '<form action="xoa_user.php" method="post">
