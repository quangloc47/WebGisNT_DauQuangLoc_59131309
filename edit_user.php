...
<body id="view_user">
	...
	// Trang này là để chỉnh sửa hồ sơ người dùng.
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (empty($errors)) { // Nếu không lỗi
			$qud = "UPDATE public.user SET hoten='$fn', diachi='$e', sdt='$dob', avatar='$at', gtinh='$s' WHERE tentk='$id'";
	...
	if (pg_num_rows($r) == 1) { // ID người dùng hợp lệ, hiển thị biểu mẫu.
		...
		// Tạo biểu mẫu:
		echo '<form action="edit_user.php" method="post" style="width: 900px; margin: 0 auto" enctype="multipart/form-data">
        <table class="table table-borderless table-success" align="center" style="width: 900px; margin-top: 20px;">
          ...
          <tr> 
            <td rowspan="7">
            	<img src="images/' . $row[3] . '" width="250" height="350" />
